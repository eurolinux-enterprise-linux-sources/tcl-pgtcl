 ...
 *
 * IDENTIFICATION
 *	  $Id: pgtclCmds.c,v 1.9 2007/12/28 02:16:54 lbayuk Exp $
 *
 *-------------------------------------------------------------------------
 */
...
constant, like 'S'. See postgres_ext.h for the list.
 For compatibility with Gborg pgtcl-1.4, these changes have been made:
    Accept field names without MESSAGE_, SOURCE_, STATEMENT_ prefix.
    Do case insensitive compares (Gborg pgtcl uses lower case names only,
and previous versions of this code used upper case only.)
...
pg_result_errorfield_code(char *fieldName)
{
	static struct errorfield_names_t {
        ...
        /* These are for compatibility with Gborg pgtcl-1.4: */
	...
	if (fieldName[1])
	{
		...
		/* Check for single-character code match, case insensitive. Note
			these are being checked against the PG_DIAG_* values, which
			are defined in postgres_ext.h as single upper-case chars.
		*/
		field1 = toupper(fieldName[0]);
 ...
 * pg_result
 ...
 the options are:
	...
	-errorField ?code?
		...
		Note: -errorField was introduced first, but for compatibility with
		Gborg pgtcl 1.4 the error field code was made optional to -error
		and now we implement both of these as the same.
...
Pg_result(ClientData cData, Tcl_Interp *interp, int objc, Tcl_Obj *CONST objv[])
{
	...
	switch ((enum options) optIndex)
	{
		...
		default:
			/*
			 * Note: This should never happend, since Tcl_GetIndexFromObj
				   ...
				   PGresult *result, int tupno)
{
	 ...
	 * "may be used uninitialized", but that is not true.
	 */
	char	   *varname;
	char	   *indexname;
...
"SEEK_CUR", "SEEK_END", or "SEEK_SET"
 ...
 returns: the new position in the object, but this isn't currently documented
 on error: throws a Tcl error.
***********************************/
int
			...
			Tcl_Obj *CONST objv[])
{
	...
	/* Note: undocumented but true, lo_lseek returns -1 on error */
	if (newOffset == -1)
	{
		   ...
		   Tcl_Obj *CONST objv[])
{
	...
	/* Note: lo_tell returns -1 on error, but is documented to return
		a negative number on error. I would rather test for -1, but:
	*/
	if (offset < 0)
	{
...
Pg_select(ClientData cData, Tcl_Interp *interp, int objc, Tcl_Obj *CONST objv[])
{
	...
	/* What is this for? Why was it inside the loop? -ljb */
	...
	for (tupno = 0; tupno < PQntuples(result); tupno++)
	{
		...
		for (column = 0; column < ncols; column++)
		{
			...
			valueObj = result_get_obj(result, tupno, column);
			Tcl_IncrRefCount(valueObj);
...
Pg_quote
    ...
    Note: First there was pg_escape_string, based on libpq PQescapeString.
    This was added to Gborg libpgtcl CVS after 1.4b3. About a year later,
    pg_escape_string was removed and pg_quote was added instead. But both
    pgintcl and pgtcl-ng already released a version with pg_escape_string.
    I'm not about to break things, so both are supported.
 ...
 * Pg_escape_bytea
   ...
   See libpq PQescapeBytea.
   In summary, this produces octal escapes as \\nnn (with 2 literal
   backslashes before) and \' for single quote and \\\\ (4 backslashes)
   for backslash. The doubling of backslashes is due to PostgreSQL
   parsing the data once for SQL syntax, and again for bytea input.
				 ...
				 Tcl_Obj *CONST objv[])
{
	...
	/*
	 * Get the source data as a Tcl ByteArray object
	 */
	 ...
	 * (The libpq documentation doesn't say, but this can return NULL.)
	 */
	to_string = PQescapeBytea(from_binary, from_len, &to_len);
	if (!to_string)
	{
				 ...
				 Tcl_Obj *CONST objv[])
{
	 ...
	 */
	to_binary = PQunescapeBytea(Tcl_GetString(objv[1]), &to_len);
	if (!to_binary)
	{
